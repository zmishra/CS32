...
void insertion_sort(vector<Student>& s, bool comp(const Student&, const Student&))
{
	...
	{
		Student nextStudent = s[unsorted];
		int loc = unsorted;
		while ((loc > 0) && comp(nextStudent, s[loc - 1]))
		{
			s[loc] = s[loc - 1];
			loc--;
		}
...
int main()
{
	...
	// destruction time.
	{
		...
		vector<Student*> auxStudentsPtr;
		for (int i = 0; i < auxStudents.size(); i++)
			auxStudentsPtr.push_back(&auxStudents[i]);

		sort(auxStudentsPtr.begin(), auxStudentsPtr.end(), compareStudentPtr);

		for (int i = 0; i < students.size(); i++)
			students[i] = *auxStudentsPtr[i];
